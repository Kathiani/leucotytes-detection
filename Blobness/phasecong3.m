...
function [covx2 covy2 covxy] = phasecong3(varargin)
...
% Get arguments and/or default values    
    ...
    for o = 1:norient                    % For each orientation...
        ...
        for s = 1:nscale,                  % For each scale...
            ...
            Ffilter{s,o} = logGabor{s} .* spread;                                % components to get the filter.                                  
        ...
        sumAn = sumAn + sumAn_ThisOrient; %Somátório das amplitudes de todas as orientações;
        sumEnergy = sumEnergy + (weight.*Energy); %Somatório de energia em cada orientação;
    ...
    %pcSum = pcSum/max(max(pcSum));
     pcSum = sumEnergy./(sumAn + epsilon); 
    ...
    %for k=1:420
        %for l=1:592
         %denom = sqrt(covxy(k,l).^2 + (covx2(k,l)-covy2(k,l)).^2)+epsilon;
         %M = (covy2(k,l)+covx2(k,l) + denom)/2;          % Maximum moment
         %m = (covy2(k,l)+covx2(k,l) - denom)/2;          % ... and minimum moment
         %display(m);
        %end
   %end
    ...
    %Plot filter bank
    %h = figure(5); x = 1;
    %set(h,'name',sprintf('Filters: scale = %.f  e orient = %.f',s,o));
    %for i=1:s
        %for j=1:o
           %subplot(norient,nscale,x),imagesc(ifftshift(Ffilter{i,j})), axis image, colormap(gray)
           %title(sprintf('s=%.f & o=%.f',i,j));
           %x = x + 1;
        %end
    %end
...
function [im, nscale, norient, minWaveLength, mult, sigmaOnf, ...
    ...
    %minWaveLength   = 8;     % Wavelength of smallest scale filter.    
    %mult            = 2;   % Scaling factor between successive filters.    
    %sigmaOnf        = 1.2;  % Ratio of the standard deviation of the
                             % Gaussian describing the log Gabor filter's
                            % transfer function in the frequency domain
                             % to the filter center frequency.    
    %k               = 2;   % No of standard deviations of the noise
                             % energy beyond the mean at which we set the
                             % noise threshold point. 
    %cutOff          = 0.4;   % The fractional measure of frequency spread
                             % below which phase congruency values get penalized.
    %g               = 10;    % Controls the sharpness of the transition in
                             % the sigmoid function used to weight phase[M E B] = main('video.avi')
                             % congruency for frequency spread.  
